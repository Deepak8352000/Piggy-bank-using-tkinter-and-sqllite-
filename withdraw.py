...
class Withdraw(Toplevel):
    def __init__(self,accnum1):
        ...
        self.buttom=Frame(self , height=550,bg='#3477eb')
        ...
        self.heading=Label(self.top,text="Withdraw money",
        ...
        #withdraw
        self.label_withdraw=Label(self.bin1,text="Enter amount of withdraw",font='arial 12 bold',fg='black',bg='light blue')
        self.label_withdraw.place(x=10,y=120)
 
        self.entry_withdraw=Entry(self.bin1,width=30,bd=4)
        self.entry_withdraw.place(x=10,y=160)

        button=Button(self.bin1,text='  Withdraw  ',fg='black',font='arial 10 bold',bg="light blue",command=self.withdraw_money)
    ...
    def withdraw_money(self):
            withdarwmoney=self.entry_withdraw.get()
            curbal=self.accmoney1
            totalmoney=int(curbal)-int(withdarwmoney)
            if totalmoney<0:
                messagebox.showinfo("ALERT",'You cannot Withdarw more than current balence')
            else:
                ...
                try:
                    cur.execute(query)
                    con.commit()
                    messagebox.showinfo("Updated","Accounted Updated")
                    self.destroy()
